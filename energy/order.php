...
fwrite ($myfile, "Spieler: $username Braucht einen Strom Anschluss.</br>");
fclose($myfile);
$timestamp = time();
...
fclose($myfile);
	...
	fwrite ($myfile, "Spieler: $username Bestellte Stomanschluss bei AKW Buchwald(WEB)</br>");
	fclose($myfile);
	$timestamp = time();
	$datum = date("d.m.y-H:i", $timestamp);
	...
	if ($rcon->connect())
	{
	$rcon->sendCommand("tell $username Bestellung wurde Gemeldet, bitte habe Gedult.");
	$rcon->sendCommand("tell $energyManager $username braucht Strom -> neuer Auftrag.");
	}
?>
<?php 
