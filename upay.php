...
<?php
	...
	$myfile = fopen("/var/www/html/daten/bank/$suser-$uid-date.html", "a");
	fwrite ($myfile, $datum. "</br>");
	fclose($myfile);
	//schreibt die betrag ins Doc.
	$myfile = fopen("/var/www/html/daten/bank/$suser-$uid-in.html", "a");
	fwrite ($myfile, $stdlohn. " € + </br>");
	fclose($myfile);	
	//schreibt die verwendungs Zweck ins Doc.
	$myfile = fopen("/var/www/html/daten/bank/$suser-$uid-vz.html", "a");
	fwrite ($myfile, "Lohn vom Staat</br>");
	fclose($myfile);	
	//schreibt die ausgabe Zweck ins Doc.
	$myfile = fopen("/var/www/html/daten/bank/$suser-$uid-out.html", "a");
	fwrite ($myfile, "&nbsp;" ."</br>");
	fclose($myfile);
	//GS Admin Debug
	$myfile = fopen("/var/www/html/daten/log/spieler/$suser-log.html", "a");
	fwrite ($myfile, "Spieler: $suser Bekam Lohn $stdlohn €(APP)</br>");
	fclose($myfile);
	$timestamp = time();
	$datum = date("d.m/H:i", $timestamp);
	//schreibt die Zeit ins Doc.
